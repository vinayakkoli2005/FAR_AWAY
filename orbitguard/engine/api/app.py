...
def _simulated_dodge_event(asset: CatalogObject, cfg: ScreeningConfig) -> dict | None:
    """A clearly-badged TRAINING event exercising the CDM-grade path.

    Public TLE data can never produce a DODGE (worst-case bounds only clear or
    escalate), so live demos would never show the red path. This event is
    simulated GEOMETRY with realistic CDM covariance — but the probability,
    ...
    event = ConjunctionEvent(
        asset_id=asset.norad_id, object_id=99999, tca=tca,
    ...
    fake_obj = SimpleNamespace(
        norad_id=99999, name="SIM-DEBRIS (TRAINING)", epoch=tca,
...
def _run_screening(asset_ids: list[int], days: float) -> dict:
    ...
    if os.environ.get("ORBITGUARD_DEMO", "1") == "1" and asset_ids:
        sim = _simulated_dodge_event(STATE.by_id[asset_ids[0]], cfg)
        if sim is not None:
            events.insert(0, sim)
    return {
